...
class Solution {
    public List<Integer> topKFrequent(int[] nums, int k) {
        ...
        Map<Integer,Integer> numMap = new HashMap<Integer,Integer>();
        Map<Integer,Integer> revNumMap = new HashMap<Integer,Integer>();
        int maxlen = -1;
        for(int i=0; i<nums.length; i++){
            if(numMap.isEmpty() || numMap.get(nums[i])==null){
                ...
                numMap.put(nums[i],numMap.get(nums[i])+1);
                maxlen = Math.max(maxlen,numMap.get(nums[i]));
            }
        }
        
        for(Map.Entry e:numMap.entrySet()){
            revNumMap.put((Integer)e.getValue(),(Integer)e.getKey());
        }
        ...
        while(k!=0){
            int number = maxlen;
            res.add(revNumMap.get(maxlen));
            k--;
            maxlen--;
            while(!revNumMap.containsKey(maxlen)) maxlen--;
        }
        return res;
    }
}